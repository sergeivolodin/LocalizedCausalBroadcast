/** @file boundedthreadedreceiver.h
 * @brief This file implements a receiver which delivers
 * its messages in a separate thread with a limit on buffer size
 ...
 * its messages in a separate thread with a limit on buffer size */
class BoundedThreadedReceiver : public Receiver
{
private:
    /// @brief Buffer data
...
public:
     ...
     * @param buffer_size Maximal receive queue length
     */
    BoundedThreadedReceiver(unsigned this_process, unsigned buffer_size = 100, Target* target = nullptr);

    /**
     * @brief canDeliver Returns true if subsequent deliverToAll call would not block.
     * Call from the same thread as deliverToAll (single)
     * @return True if can add message to the queue in non-blocking way
     */
    bool canDeliver();

    virtual ~BoundedThreadedReceiver() {}
};
